...
var isAngleEN, isAngleES, isAngleSE, isAngleSW, isAngleWS, isAngleWN, isAngleNW : boolean
...
loop
	...
	finalAngle := 90 - userAngle
	finalAngle := 90 + userAngle
	finalAngle := 180 - userAngle
	finalAngle := 180 + userAngle
	finalAngle := 270 - userAngle
	finalAngle := 270 + userAngle
	finalAngle := 360 - userAngle
	
	isAngleEN := finalAngle > 45 and finalAngle < 90
	isAngleES := finalAngle > 90 and finalAngle < 135
	isAngleSE := finalAngle > 135 and finalAngle < 180
	isAngleSW := finalAngle > 180 and finalAngle < 225
	isAngleWS := finalAngle > 225 and finalAngle < 270
	isAngleWN := finalAngle > 270 and finalAngle < 315
	isAngleNW := finalAngle > 315 and finalAngle < 360
    end if
    %Outputs
    if (userChoiceBear) then
	...
	if (isAngleEN) then
	    put "Compass E", userAngle, chr(186), "N is bearing ", finalAngle
	elsif (isAngleES) then
	    put "Compass E", userAngle, chr(186), "S is bearing ", finalAngle
	elsif (isAngleSE) then
	    put "Compass S", userAngle, chr(186), "E is bearing ", finalAngle
	end if    
    end if
    %End of Program
